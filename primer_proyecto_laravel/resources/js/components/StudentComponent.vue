    ...
    <div class="grid grid-cols-1 gap-4 py-2 shadow-lg rounded-lg bg-gray-200/40">
        ...
        <div class="container mx-auto border-2 border-stone-600 rounded-t-lg bg-neutral-800 w-11/12 h-fit">
            <div class="flex justify-between items-center p-4 bg-neutral-800 rounded-t-lg text-white overflow-hidden">
                ...
                <div class="flex items-center">
                    <button data-target="form" @click="showHide(this)" class="w-14 h-14 bg-neutral-800 hover:bg-neutral-700 ease-out duration-300 text-white font-bold py-2 px-4 rounded-full">
            ...
            <form method="post" @submit.prevent="saveChanges" @reset.prevent="clearForm" id="form" class="bg-neutral-300 overflow-hidden h-fit">
                <div class="flex flex-wrap p-4">
                    <div class="w-full md:w-1/2 mb-1 px-4 flex items-center">
                        ...
                        <input v-model="student.code" type="text" required pattern="[0-9]{3}" title="Ingrese el código de estudiante" placeholder="Código" class="shadow appearance-none border rounded-r-lg w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                    </div>
                    <div class="w-full md:w-1/2 mb-1 px-4 flex items-center">
                    ...
                    <div class="w-full md:w-1/2 mb-1 px-4 flex items-center">
                        ...
                        <input v-model="student.last_name" type="text" required pattern="(^[A-ZÁÉÍÓÚÑ]{1}[a-záéíóúñ]{1,20})+(([\s][A-ZÁÉÍÓÚÑ]{1}[a-záéíóúñ]{1,20})+)*$" title="Ingrese el apellido del alumno" placeholder="Apellido del alumno" class="shadow appearance-none border rounded-r-lg w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                    </div>
                    <div class="w-full md:w-1/2 mb-1 px-4 flex items-center">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="h-full w-8 rounded-l-lg bg-gray-500 fill-gray-200 stroke-1 stroke-black">
        ...
        <div class="container mx-auto border-2 border-stone-600 rounded-t-lg w-11/12 h-fit bg-neutral-800">
            <div class="flex justify-between items-center p-4 bg-neutral-800 rounded-lg text-white overflow-hidden">
                ...
                <div class="flex items-center">
                    <button data-target="table" @click="showHide(this)" class="w-14 h-14 bg-neutral-800 hover:bg-neutral-700 ease-out duration-300 text-white font-bold py-2 px-4 rounded-full">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="h-6 w-6 stroke-white stroke-2 fill-transparent">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                        </svg>
                    </button>
                ...
                <table class="table-auto w-full">
                        ...
                        <tr>
                            <th colspan="9" class="px-4 py-2 rounded-tl-lg bg-gray-200/25 duration-500">
                                <div class="flex justify-between items-center">
                                    <svg @click="searchStudent" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="h-10 w-8 rounded-l-lg bg-white hover:bg-gray-300 duration-500 fill-gray-100/25 stroke-2 stroke-black">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                                    </svg>
                                    <input @keyup="searchStudent" v-model="word" type="text" placeholder="Buscar" class="w-full h-10 px-4 py-2 bg-gray-100 border-2 border-transparent rounded-r-lg text-gray-700">
                                </div>
                            </th>
                        </tr>
                        <tr class="text-white">
                    ...
                    <tbody>
                        <tr v-for="student in students" @click="showEstudent(student)" :key="student.idStudent" class="text-black hover:bg-gray-400 duration-500">
                            ...
                            <td class="bg-gray-100 hover:bg-gray-200 duration-500 border-l px-4 py-2">{{ student.last_name }}</td>
...
<script>
    export default {
        ...
        methods: {
            getData() {
                ...
                this.students = this.students.filter(student => {
                    if (this.word.length > 0) {
                        if (student.name.toLowerCase().includes(this.word.toLowerCase()) || student.last_name.toLowerCase().includes(this.word.toLowerCase()) || student.code.toLowerCase().includes(this.word.toLowerCase())) {
                            return student;
                        }
                    } else {
                        return student;
            ...
            async syncData(data='', method='POST', url='students') {
                await axios({
                    method,
                    url,
                    data,
                ...
                .catch(err => {
                    this.student.showMsg = true;
                    this.student.msg = 'Ha ocurrido un error';
                });
            },
            saveChanges() {
                this.getData();
                let student = this.students || [],
                    method = 'PUT',
                    url = `students/${this.student.idStudent}`;
                ...
                this.syncData(this.student, method, url);
                this.student.showMsg = true;
